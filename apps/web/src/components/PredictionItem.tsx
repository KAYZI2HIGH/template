...
interface PredictionItemProps {
  pred: UserPrediction;
  index: number;
  roomStatus: "waiting" | "started" | "completed" | "unknown";
  room: Room | undefined;
...
const getStatusColor = (status: "waiting" | "started" | "completed" | "unknown") => {
  switch (status) {
    case "waiting":
...
const getStatusBarColor = (status: "waiting" | "started" | "completed" | "unknown") => {
  switch (status) {
    case "waiting":
      return "bg-gray-500";
    case "started":
...
export function PredictionItem({
  ...
  const isSettled = roomStatus === "completed";

  // Hook is called at component level, not in a map
  const { data: userOutcome, isLoading, error } = usePredictionOutcome(
    isSettled ? pred.roomId : undefined,
    walletAddress
  );
  ...
  console.log(`🎯 PredictionItem [${index}]`, {
    ...
    isSettled,
    walletAddress,
    shouldFetchOutcome: isSettled ? true : false,
    ...
    >
      ...
      <div className="flex justify-between items-start mb-2">
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Prediction: <span className="text-green-300">{pred.prediction}</span>
          </p>
        </div>
        <div className="flex flex-col gap-1 items-end ml-2">
          {/* Outcome Badge (WIN/LOSS) */}
          {isSettled && userOutcome && (
            <span
      ...
      {isSettled && (
        <button
          ...
          className="mt-2 w-full bg-yellow-500/10 border border-yellow-500/50 hover:bg-yellow-500/20 transition-all duration-300 text-yellow-300 py-2 px-3 rounded text-xs font-medium"
        >
          Settle & Claim
        </button>
      )}
