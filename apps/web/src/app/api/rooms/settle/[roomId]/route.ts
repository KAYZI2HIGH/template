...
export async function POST(
  ...
  try {
    ...
    // Verify room time has actually elapsed before allowing settlement
    // Anyone can settle once time is up (prevents griefing by owner)
    if (room.status === "started" && room.ends_at) {
      const roomEndTime = new Date(room.ends_at).getTime();
      const currentTime = Date.now();

      if (currentTime < roomEndTime) {
        const timeRemaining = Math.ceil((roomEndTime - currentTime) / 1000);
        return Response.json(
          {
            error: `Room is still active. Cannot settle for ${timeRemaining} more seconds`,
            timeRemaining,
          },
          { status: 400 }
        );
    ...
    // Check if room is already completed or settled
    ...
    // Verify user participated in the room (has a prediction)
    // This ensures only actual players can trigger settlement, preventing spam
    const { data: userPrediction, error: predError } = await supabase
      .from("predictions")
      .select("id")
      .eq("room_id", roomId)
    ...
    // Allow settlement if: user is room creator OR user has a prediction in the room
    const isCreator = room.creator_wallet_address === auth.wallet_address;
    const isParticipant = !!userPrediction;

    if (!isCreator && !isParticipant) {
      return Response.json(
        { error: "You must be the room creator or a participant to settle" },
        { status: 403 }
      );
    }
    ...
    // Step 7: Update room status to completed (NOT settled)
    // Room will be marked as settled after winners have claimed payouts
    console.log(`✅ Marking room as completed (awaiting winner claims)...`);
    ...
    const { error: roomUpdateError } = await supabase
      ...
      .update({
        ...
        ending_price: endingPrice,
        updated_at: new Date().toISOString(),
        // DO NOT mark as settled yet - winners need to claim first
      })
      .eq("id", roomId);
